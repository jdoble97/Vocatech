...
CREATE TABLE `baraja` (
  ...
  PRIMARY KEY (`ID`),
  KEY `email` (`email`),
  CONSTRAINT `baraja_ibfk_1` FOREIGN KEY (`email`) REFERENCES `usuarios` (`email`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `carta` (
  `ID` int NOT NULL AUTO_INCREMENT,
  `barajaID` int NOT NULL,
  `nombreCastellano` varchar(50) DEFAULT NULL,
  `nombreIngles` varchar(50) DEFAULT NULL,
  `aprendido` tinyint(1) DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `barajaID` (`barajaID`),
  CONSTRAINT `carta_ibfk_1` FOREIGN KEY (`barajaID`) REFERENCES `baraja` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `puntuaciones` (
  ...
  `idVocabulario` varchar(50) DEFAULT NULL,
